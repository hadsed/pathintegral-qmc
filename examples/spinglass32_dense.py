...
Date: 12.11.14
Description: Test the performance of PIQMC code by comparing with 
             a presolved Ising instance from M. Juenger's spin
             glass server. Here we're using inst_0_32x32.txt,
             generated by gen_ising.py. This version tests the
             dense SA and QA routines. Spin glass server 
...
tannealingsched = np.linspace(preannealingtemp,
                              ...
                              annealingsteps)
t0 = time.time()
sa.Anneal_dense(tannealingsched, annealingmcsteps, 
                spinVector, isingJ.todense(), rng)
t1 = time.time()
...
annealingsched = np.linspace(fieldstart,
                             ...
                             annealingsteps)
t0 = time.time()
sa.Anneal_dense(preannealingsched, 1, spinVector, isingJ.todense(), rng)
qmc.QuantumAnneal_dense(annealingsched, annealingmcsteps,
                        trotterslices, annealingtemp, nspins, 
                        configurations, isingJ.todense(), rng)
t1 = time.time()
