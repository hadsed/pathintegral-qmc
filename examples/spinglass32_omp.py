...
File: spinglass32_omp.py
Author: Hadayat Seddiqi
Date: 01.22.15
Description: Do the 32x32 square lattice Ising system
             using OpenMP directives in Cython to do
             parallel spin updates. The problem is the 
             same as the non-MPI version, with the same 
             answer.
...
for i in range(1,5):
    ...
    t0 = time.time()
    sa.Anneal_parallel(tannealingsched, annealingmcsteps, 
                       spinVector, neighbors, i)
    t1 = time.time()
    print ("Final energy: ", 
           sa.ClassicalIsingEnergy(spinVector, isingJ))
    print ("Residual: ", 
